...
public abstract class BaseService {
	...
	private TokenBuilder getTokenbuilder() {
		...
		{
			tokenBuilder = TokenBuilder.getInstance(getLinkID(), getSecretKey())
									.ServiceID(isTest ? ServiceID_TEST : ServiceID_REAL)
									.addScope("member");
			
			for(String scope : getScopes()) tokenBuilder.addScope(scope);
		}
	...
	private String getSessionToken(String CorpNum,String ForwardIP) throws PopbillException {
		...
		if(CorpNum == null || CorpNum.isEmpty()) throw new PopbillException(-99999999,"회원 사업자번호가 입력되지 않았습니다.");
    	
		Token token = null;
		
		if(tokenTable.containsKey(CorpNum)) token = tokenTable.get(CorpNum);
		...
		if(expired) {
			if(tokenTable.containsKey(CorpNum)) tokenTable.remove(CorpNum);
	...
	/**
	 * 연동회원 잔여 포인트 확인.
	 * @param CorpNum Member's CorpNum
    ...
    /**
     * 연동회원 가입요청.
     * @param joinInfo Join Information. see JoinForm
    ...
    /**
     * 연동회원의 가입여부 확인. 
     * @param CorpNum CorpNum to Check.
     * @param LinkID LinkId to Check.
    ...
    protected <T> T httppost(String url , String CorpNum ,String PostData, String UserID , String Action, Class<T> clazz) throws PopbillException
    {
    	...
    	if((PostData == null || PostData.isEmpty()) == false) {
	    	...
	    	try {
				...
				output.flush();
				output.close();
			} catch (Exception e) {throw new PopbillException(-99999999, "Fail to POST data to Server.",e);}
		...
		try {
			...
			if(error == null)
				throw new PopbillException(-99999999, "Fail to receive data from Server.",e);
			else
	...
	protected class UnitCostResponse {
		...
		public float getUnitSost() {return unitCost;}
	}
