...
public class TaxinvoiceServiceTEST {
    ...
    @Test
        public void RegistIssue_TEST() throws PopbillException {
    		try {
	            ...
	            taxinvoice.setWriteDate("20230727"); // 필수, 기재상 작성일자
	            ...
	            taxinvoice.setInvoicerMgtKey("20230727-004"); // 공급자 발행까지 API로 발행하고자 할경우 정발행과
	    ...
	    //        taxinvoice.setOrgNTSConfirmNum("20191008888888880000003e"); // 수정세금계산서 작성시 원본세금계산서의 ItemKey기재. ItemKey는 문서확인.
	    		...
	    		// FLE(FieldLevelEncryption) 적용된 공동인증서 PublicKey 
	    		//taxinvoice.setCertPublicKey("MIIblabla~");
	            
	            // FLE(FieldLevelEncryption) 적용된 공동인증서 PrivateKey
	    		//taxinvoice.setCertPrivateKey("MIIblabla~");
	            
	            // FLE(FieldLevelEncryption) 적용된공동인증서 비밀번호
	    		//taxinvoice.setCertCipher("MIIblabla~");
    ...
    public void search_TEST() throws PopbillException {
        ...
        response = taxinvoiceService.Search("1234567890", MgtKeyType.SELL, DType,
                ...
                QString, Page, PerPage, Order, interOPYN, RegType, CloseDownState, MgtKey, null);
